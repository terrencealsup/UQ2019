function [ A, f, gridPts ] = genOperators2D( a, level )
% Dirichlet top
...
for j=0:N-2
    for i=0:N-1
        ...
        for ptsIter=1:size(ptsList, 1)
            ...
            A(gIndx, ggIndx) = integral2(@(x, y)a(x, y).*dphiProd(x, y, i, j, pts(1), pts(2)), max(0, (i - 1)*h), min(1, (i + 1)*h), max(0, (j - 1)*h), min(1, (j + 1)*h));
        end
        A(gIndx, gIndx) = integral2(@(x, y)a(x, y).*dphiProd(x, y, i, j, i, j), max(0, (i - 1)*h), min(1, (i + 1)*h), max(0, (j - 1)*h), min(1, (j + 1)*h));
        % impose Neumann 1 on base
        if(j == 0)
            f(gIndx) = integral(@(x)phili(x, i), max(0, (i - 1)*h), min(1, (i + 1)*h));
        end
    end
end
% Top: Dirichlet 0
